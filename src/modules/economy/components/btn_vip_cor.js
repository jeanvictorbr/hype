...
module.exports = {
    customId: 'btn_vip_cor',
    async execute(interaction) {
        const userData = await prisma.hypeUser.findUnique({ where: { id: interaction.user.id } });
        
        // 🛡️ Apenas VIP 3 ou superior
        if (!userData || userData.vipLevel < 3) {
            return interaction.reply({ 
                content: '❌ **Acesso Negado!**\nApenas membros **VIP EXCLUSIVE (Level 3)** ou superior podem forjar cartões com cores personalizadas.', 
                ephemeral: true 
            });
        }

        // 🎨 Cria o Menu com a Opção de Resetar (Padrão) + 24 Cores
        const selectMenu = new StringSelectMenuBuilder()
            .setCustomId('select_vip_cor')
            .setPlaceholder('Forje a cor do seu Cartão VIP...')
            .setMinValues(1)
            .setMaxValues(2)
            .addOptions([
                { label: '🔄 RESTAURAR PADRÃO', description: 'Voltar para a cor original do seu nível VIP', value: 'padrao', emoji: '💳' },
                ...
                { label: 'Cinza Asfalto', description: 'Neutro e urbano', value: '#2F4F4F', emoji: '🖤' }
            ]);

        const row = new ActionRowBuilder().addComponents(selectMenu);

        await interaction.reply({ 
            content: '💳 **Forja do Cartão VIP**\n\nSelecione:\n👉 **Padrão:** Devolve o cartão à cor original.\n👉 **1 Cor:** Todo o cartão nessa cor.\n👉 **2 Cores:** A primeira é o Brilho Principal, a segunda é o Fundo!', 
            components: [row], 
            ephemeral: true 
        });
    }
